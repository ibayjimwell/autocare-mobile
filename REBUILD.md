...
3.  **SELECTIVE APPLE GLASSMORPHISM & MATERIALS:**
    *   **Rule:** Use Glassmorphism ONLY on floating or overlapping UI elements suggested by the inspiration image (e.g., a sticky navigation bar, floating icon buttons, floating CTA action bars, status badges, or popover modals) over colored/dark backgrounds (like `bg-primary`) or images.
        *   **Glass Icon Buttons / Badges:** `w-11 h-11 rounded-full bg-white/15 border border-white/20 items-center justify-center min-h-[44px] min-w-[44px]`.
        *   **Sticky/Floating Header Bars:** Translucent surface (`bg-white/15`) with a subtle border (`border-b border-white/20` or `border border-white/20`).
        *   **Fixed Floating Bottom CTA Bars:** Floating above content with `bg-white/15 border border-white/20 rounded-2xl p-4 shadow-lg`.
        *   **Floating Modals / Action Sheets / Overlay Badges:** Glass surfaces with `bg-white/15 border border-white/20` and drop shadows (`shadow-md` / `shadow-lg`). Text on glass elements should be `text-white` or `text-white/70`.
    *   **Constraint:** DO NOT apply glassmorphism to standard body text containers, input fields, or standard scrollable grouped list cards.
...
6.  **ICONS:**
    *   **Rule:** Use `lucide-react-native` for all icons. Replace existing icon libraries (Ionicons, FontAwesome, etc.) with the closest equivalent Lucide icon. Standard icon color should be the foreground or muted-foreground color, unless it is a primary action color `#C1272D` or white (`#FFFFFF`) over primary glass surfaces.
...
7.  **AUTOCARE DESIGN SYSTEM (APPLE HIG) SPECIFICATIONS:**
    ...
    *   **Glass Material Surface:** `bg-white/15 border border-white/20` (Translucent Glass).
    ...
    *   **Text Hierarchy:**
        *   Primary / Body: `text-foreground` (`#000000`) or `text-white` on dark/primary surfaces.
        *   Secondary / Captions / Placeholders: `text-muted-foreground` (`#8E8E93`) or `text-white/70` on dark/primary surfaces.
    ...
    *   **Touch Targets & Spacing:**
        *   All interactive buttons MUST be at least 44x44pt (e.g., `min-h-[44px] min-w-[44px]` or `py-4`).
        *   Circular icon buttons must be `w-11 h-11 rounded-full`.
        *   Horizontal margins from screen edges: 16px (`px-4` or `mx-4`).
    *   **Border Radius (Squarcles):**
        *   Glass Icon Buttons: `rounded-full`.
        *   Text inputs: `rounded-lg` (10px).
        *   Cards / primary buttons: `rounded-xl` (14px).
        *   Modals / Bottom sheets / Floating Bars / Hero headers: `rounded-2xl` (20px), `rounded-3xl` (24px), or `rounded-b-[32px]`.
